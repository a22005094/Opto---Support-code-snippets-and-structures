...
namespace Assets.Models.Managers
{
    ...
    //    CRUD operations over generated application data, by abstracting Elasticsearch API access into simple method calls.
    //
    //  - This level of abstraction is achieved by using NEST, a .NET Package that wraps the REST API calls into
    //    a familiar "remote method calls using Objects" fashion, providing easier API integration, strongly-typed
    //    requests and responses, while also providing side documentation about most Types and Method calls shipped with the package.
    //
    ...
    public sealed class ESManager
    {
        // Singleton instance to this manager
        private static ESManager _instance = null;

        // Lock object is being used to help ensure thread safety during singleton initialization
        private static readonly object _lockObj = new object();

        // The actual Elasticsearch client (NEST) that will communicate with the back-end server.
        private static ElasticClient _elasticClientInstance = null;

        // Names of the Indices to store data @ Elasticsearch
        public const string EXERCISES_INDEX = "exercises"; // the name of the Index where exercise data is stored
        public const string SESSIONS_INDEX = "sessions"; // the name of the Index where session  data is stored
        ...
        public async void CreateIndexFromObject<T>(string indexName, T sampleObject) where T : class
        {
            ...
            try
            {
                ...
                var response =
                    await _elasticClientInstance.Indices.CreateAsync(indexName,
                        selector => selector.Map<T>(m => m.AutoMap()));
        ...
        {
            // Responsible for sending an Exercise object to Elasticsearch server.
            ...
            IndexResponse response =
                await _elasticClientInstance.IndexAsync(exercise, selector => selector.Index(EXERCISES_INDEX));
        ...
        {
            // Responsible for sending a Session object to Elasticsearch server.
        ...
        // * Secondary methods
        //  These are only for seldom use, and outside of the expected flow of the project.
        //  The focus is mainly to help during development stages.

        public async void DeleteIndex(string indexName)
        {
            var resp = await _elasticClientInstance.Indices.DeleteAsync(Indices.Parse(indexName));
            Debug.Log(resp.IsValid ? $"Index {indexName} deleted successfully" : $"Failed to delete index {indexName}");
        }
    }
}